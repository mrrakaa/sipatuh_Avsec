        ...
        <div class="bg-white shadow-md rounded-lg">
                        ...
                        @if (Auth::check() && Auth::user()->role === 'superadmin')
                            <div class="relative" x-data="{ showDatePicker: false }">
                                <button @click="showDatePicker = !showDatePicker"
                                    class="w-full sm:w-auto bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors duration-150 flex items-center justify-center space-x-2">
                                ...
                                <div x-show="showDatePicker" @click.away="showDatePicker = false"
                                    ...
                                    <form action="{{ route('generate.merged.pdf') }}" method="POST" class="space-y-4">
                                        @csrf
                                        <div>
                                        ...
                                        <div>
                                            <label for="pdf_end_date" class="block text-sm font-medium text-gray-700 mb-1">
                                                Tanggal Akhir
                                            </label>
                        ...
                        <div class="relative w-full sm:w-auto" x-data="{ isOpen: false }">
                            ...
                            <div id="filterDropdown"
                                ...
                                <div class="p-4">
                                    ...
                                    <form id="dateFilterForm" class="space-y-4" action="{{ route('filter.hbscp.forms') }}"
                                        method="POST">
                                        @csrf
                                        <div>
            ...
            <div class="p-6">
                ...
                <div id="pending-content" class="tab-content">
                    <h2 class="text-xl font-bold text-black">Formulir HBSCP Bagasi Barat - Belum Diperiksa</h2>
                    <h4 class="text-sm font-light text-black mb-4">Daftar formulir HBSCP Bagasi Barat yang belum selesai
                        diperiksa</h4>

                    @if (isset($startDate) && isset($endDate))
                        <div class="mb-4 p-4 bg-blue-100 text-blue-700 rounded-lg">
                            <p>Menampilkan hasil dari
                                <strong>{{ \Carbon\Carbon::parse($startDate)->format('d-m-Y') }}</strong>
                                hingga <strong>{{ \Carbon\Carbon::parse($endDate)->format('d-m-Y') }}</strong>.
                            </p>
                            <a href="{{ route('xrayform') }}" class="text-blue-600 underline hover:text-blue-800">Reset
                                Filter</a>
                    ...
                    @if ($allHbscpForms->where('status', 'pending_supervisor')->count() > 0)
                        <div class="overflow-x-auto">
                            <table class="w-full border-collapse bg-white rounded-lg overflow-hidden">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($allHbscpForms->where('status', 'pending_supervisor') as $form)
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4">
                                                <a href="{{ route('review.pscp.reviewpscp', $form->id) }}"
                    ...
                    @else
                        <div class="text-center py-8">
                            <p class="text-gray-500">Tidak ada formulir HBSCP Bagasi Barat yang belum diperiksa saat ini.
                            </p>
                        </div>
                    @endif
                </div>
                ...
                <div id="all-content" class="tab-content hidden">
                    <h2 class="text-xl font-bold text-black">Semua Formulir HBSCP Bagasi Barat</h2>
                    <h4 class="text-sm font-light text-black mb-4">Daftar lengkap semua formulir HBSCP Bagasi Barat</h4>
                                        ...
                                        @else bg-yellow-100 text-yellow-800 @endif">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex space-x-2">
                                                    <a href="{{ route('review.hbscp.reviewhbscp', $form->id) }}"
                                                    ...
                                                    @if ($form->status == 'approved' && Auth::check() && Auth::user()->role === 'superadmin')
                                                        <a href="{{ route('pdf.hbscp', $form->id) }}" target="_blank"
                    ...
                    @else
                        <div class="text-center py-8">
                            <p class="text-gray-500">Tidak ada formulir HBSCP Bagasi Barat saat ini.</p>
                        </div>
                    @endif
                </div>

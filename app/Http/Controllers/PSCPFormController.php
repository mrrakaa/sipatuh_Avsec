...
class PSCPFormController extends Controller
{
  public function store(Request $request)
  {
    Log::info('Received PSCP data:', $request->all());
    ...
    try {
      ...
      $pscpsave = new pscpsaved($validatedData);

      // Set user information
      if (Auth::guard('web')->check()) {
        $pscpsave->submitted_by = Auth::guard('web')->id();
        $pscpsave->officerName = Auth::user()->name;
      } elseif (Auth::guard('officer')->check()) {
        $pscpsave->submitted_by = Auth::guard('officer')->id();
        $pscpsave->officerName = Auth::guard('officer')->user()->name;
      } else {
        return redirect()->back()->with('error', 'Anda harus login untuk mengirimkan formulir ini.');
      }

      $pscpsave->supervisor_id = $validatedData['supervisor_id'];

      $pscpsave->save();
      return redirect()->route('officer.dashboard')->with('success', 'PSCP data berhasil disimpan dan menunggu persetujuan supervisor.');
    } catch (\Exception $e) {
      Log::error('Error saving PSCP data: ' . $e->getMessage());
  ...
  {
    try {
      $data = pscpsaved::findOrFail($id);
      return view('forms.pscp-show', compact('data'));
    } catch (\Exception $e) {
  ...
  {
    $form = pscpsaved::findOrFail($id);
    $supervisor = User::find($form->supervisor_id);
    return view('review.xray.reviewpscp', compact('form', 'supervisor'));
  }
  ...
  public function updateStatus(Request $request, $id)
  {
    ...
    try {
      ...
      Log::error('Error updating PSCP status: ' . $e->getMessage());
  ...
  public function update(Request $request, $id)
  {
    ...
    try {
      ...
      Log::error('Error updating PSCP form: ' . $e->getMessage());
